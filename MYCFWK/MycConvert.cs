...

{
	...
	public class MycConvert
	{
        ...
        public static Decimal cambia_punto_por_coma(string p_Number)
        {
            CultureInfo w_cult = new CultureInfo("");
        ...
        public static string CodBarrasconCAEconDigitoVerificador(string pCuit,string pTipo, string pPtoVta,string pCae, string pFechaVto )

        {
            try
            {
                ...
                int tamaño, digito = 0;
                int sumanrospares = 0;
                int sumapareseimparesx3 = 0;
                int sumanrosimpares = 0;
                int sumaimparesPor3 = 0;
                ...
                pFechaVto = MycConvert.AdaptToDateDB(pFechaVto);

                dia = pFechaVto.Substring(0, 2);
                mes = pFechaVto.Substring(3, 2);
                ano = pFechaVto.Substring(6, 4);


                pFechaVto = dia + mes + ano;

                cadena = pCuit + "0" + pTipo + pPtoVta + pCae + pFechaVto;

                for (int i = 0; i < cadena.Length; i++)
                {

                    if ((i % 2 == 0)) //impares parecen pares pero son impares porque arrancan de la posicion 1., y no 0
                    {
                        sumanrosimpares = sumanrosimpares + Convert.ToInt32(cadena.Substring(i, 1));
                    ...
                    if ((i % 2 != 0)) //pares
                    {
                        sumanrospares = sumanrospares + Convert.ToInt32(cadena.Substring(i, 1));
                    }

                }


                sumaimparesPor3 = sumanrosimpares * 3;

                sumapareseimparesx3 = sumaimparesPor3 + sumanrospares;

                resultado = Convert.ToString(sumapareseimparesx3);

                tamaño = resultado.Length;

                resultado = resultado.Substring(tamaño - 1, 1);

                digito = 10 - Convert.ToInt32(resultado);
